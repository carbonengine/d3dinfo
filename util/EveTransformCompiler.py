...
def ConvertTransforms(transformArray, parent, t, transformationsLookupDict):
    """
    Convert EveTransforms into an EveTransformArray
    """
    if t.display == False:
        print "Did not convert:", t.name, "as it is invisible"
        return
    
    if t.modifier not in [0, EVE_BOOSTER, CAMERA_ROTATION_ALIGNED,CAMERA_ROTATION, EVE_SIMPLE_HALO]:
        # ignore Booster
        print "Couldn't convert:", t.name, "as it is not a default transform (" + str(t.modifier) + ")"
        return
    elif t.useDistanceBasedScale:
...
def AreColorCurvesTheSame(a, b): 
    if a is b:
        return True
    if a.length == b.length and a.extrapolation == b.extrapolation and len(a.keys) == len(b.keys):
        keysTheSame = True
        for key1, key2 in zip(a.keys, b.keys):
            if key1.time != key2.time:
                return False
            elif key1.interpolation != key2.interpolation:
                return False
            elif not AreColorstheSame(key1.value, key2.value):
                return False
    ...
    if a.length ==b.length and a.extrapolation == b.extrapolation and len(a.keys) == len(b.keys):
        ...
        for key1, key2 in zip(a.keys, b.keys):
            if key1.time != key2.time:
                ...
                return False
            elif not AreVectorsTheSame(key1.value, key2.value):
                return False
            elif not AreVectorsTheSame(key1.left, key2.left):
                return False
            elif not AreVectorsTheSame(key1.right, key2.right):
                return False
...
def SquashCurves(curveSet):
    ...
    if 'trinity.TriColorCurve' in curvesByType:
        ...
        #print "variations", len(removableCurveMappings)
    ...
    # Since we had instances of the same curve, make sure that the unique versions
    # remain updated
    for curve in removableCurveMappings.values():
        if not curve in curveSet.curves:
